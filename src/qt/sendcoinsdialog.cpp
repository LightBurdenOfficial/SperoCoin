    ...
    platformStyle(_platformStyle)
{
    ...
    connect(ui->checkBoxSperoSend, SIGNAL(stateChanged(int)), this, SLOT(speroSendChecked(int)));
    ...
    ui->label_2->setPixmap(QPixmap(platformStyle->getThemeManager()->getCurrent()->getSendSperoSendIco()));
...
void SendCoinsDialog::on_sendButton_clicked()
{
	...
	if(CoinControlDialog::coinControl()->GetSperoSend())
	{
		questionString = questionString + " Remember with SperoSend 1% fee (minimum 0.01 SPERO) will be added to your transaction. Also your minimum balance should be more than the twice of the amount (plus fee) you send. Otherwise please use regular send.";
	}
...
void SendCoinsDialog::processSendCoinsReturn(const WalletModel::SendCoinsReturn &sendCoinsReturn, const QString &msgArg)
{
    ...
    case WalletModel::PaymentRequestExpired:
        ...
        msgParams.second = CClientUIInterface::MSG_ERROR;
        break;
    case WalletModel::SperoSendAmountExceeded:
        msgParams.first = tr("SperoSend only allows maximum of %1 now. Please reduce send amount or use regular send.").arg(BitcoinUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), MAX_ALLOWED_SPERO_SEND));
        break;
    case WalletModel::SperoSendAmountTooSmall:
        msgParams.first = tr("SperoSend only allows minimum of %1 now. Please increase send amount or use regular send.").arg(BitcoinUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), MIN_ALLOWED_SPERO_SEND));
        break;
    case WalletModel::ServiceNodesNotAvailable:
        msgParams.first = tr("SperoSend requires at least 2 anonymous service nodes available. You don't have enough service nodes connected. Please use regular-send or try later.");
        break;
    case WalletModel::AnotherSperoSendInProgress:
        msgParams.first = tr("Another SperoSend transaction still in progress. Please wait it finishes before starting a new SperoSend.");
        break;
    case WalletModel::NotEnoughReserveForSperoSend:
        msgParams.first = tr("Not enough reserve for SperoSend. You must maintain at least 2X send amount + fee. Please use regular send.");
        break;
    case WalletModel::NotSupportedSperoSendToStealthTx:
        msgParams.first = tr("You can use SperoSend or StealthAddress, but not both.");
        break;
    case WalletModel::StartSperoSendFailed:
        msgParams.first = tr("StartP2pMixerSendProcess failed.");
        break;
    case WalletModel::StartSperoSendFailedNoLegacyAddress:
        msgParams.first = tr("StartP2pMixerSendProcess failed - Create legacy addresses for and send SPERO to them for SperoSend to work.");
        break;
    case WalletModel::InvalidStealthAddress:
        msgParams.first = tr("The recipient stealth address is not valid. Please recheck.");
        break;
    case WalletModel::SperoSendWalletSyncing:
        msgParams.first = tr("SperoSend requires the wallet to be synced.");
        break;
...
void SendCoinsDialog::updateCoinControlState(CCoinControl& ctrl)
{
    ...
    if(ui->checkBoxSperoSend->isChecked()) {
    	ctrl.SetSperoSend(true);
    }
    else {
    	ctrl.SetSperoSend(false);
    }
    
    // Avoid using global defaults when sending money from the GUI
    // Either custom fee will be used or if not selected, the confirmation target from dropdown box
    ctrl.m_confirm_target = getConfTargetForIndex(ui->confTargetSelector->currentIndex());
    // SperoCoin: Disabled RBF UI
...
void SendCoinsDialog::speroSendChecked(int state)
{
    if (state == Qt::Checked)
    	CoinControlDialog::coinControl()->SetSperoSend(true);
    else
    	CoinControlDialog::coinControl()->SetSperoSend(false);
}
