...
private Q_SLOTS:
    void onFastSyncClicked();
    void onCancelButtonClicked();
    void onFastSyncOptionsClicked();
...
private:
    ...
    void updateFastSyncVisibility();

    //FastSync
    bool deflationrequested;
    Downloader m_downloader;
    GUIUtil::FastSync deepS;
    QUrl blockchain_url = QString("https://archive.sperocoin.org/files/blockchain_rebased.tar.gz");
    QString getFastSyncStatus();
    FastSyncStatus fastSyncStatus;
    fs::path tempfastSyncDir;
    void prepareInflateData(QString filename);
    fs::path tardatadir;
    bool getProxyActivated();
    void setProxyActivated(bool value);

    QMessageBox *fastSyncFinishedMessageBox;

    //FastSync options
    bool showFastSyncOptions;
    bool showFastSync;
    bool proxyActivated;
