...
// optional setting to unlock wallet for SperoSend only
// serves to disable the trivial sendmoney when OS account compromised
// provides no real security
bool fWalletUnlockSperoSendOnly = false;
...
bool CWallet::GetStakeWeight(const CKeyStore& keystore, uint64_t& nMinWeight, uint64_t& nMaxWeight, uint64_t& nWeight)
{
    ...
    if (!SelectCoinsSimple(nBalance - nReserveBalance, GetTime(), Params().GetConsensus().nCoinbaseMaturity, setCoins, nValueIn))
        return false;
...
//    CTxDB txdb("r");
    ...
    {
//      SperoCoin: Doesn't look like there was a need for this!!
//        CTxIndex txindex;
//        {
//            LOCK2(cs_main, cs_wallet);
...
bool CWallet::CreateCoinStake(const CKeyStore& keystore, unsigned int nBits, int64_t nSearchInterval, int64_t nFees, CMutableTransaction& txNew, CKey& key)
{
    ...
    if (!SelectCoinsSimple(nBalance - nReserveBalance, txNew.nTime, Params().GetConsensus().nCoinbaseMaturity, setCoins, nValueIn)) {
    ...
    /* Início Adaptação para pagamentos Foundation */
        int64_t nDevCoin;
        std::string FoundationPoSAddr = Params().GetFoundationPoSAddr();
    ...
    // Calculate coin age reward
    {
        ...
        /* Início Adaptação para pagamentos Foundation */
        if(chainActive.Height() >= HALVING_POS_03 && chainActive.Height() < END_DEV_POS_PAYMENT){
                nDevCoin = 0.25 * COIN;
                nReward -= nDevCoin;
...
if(chainActive.Height() >= HALVING_POS_03 && chainActive.Height() < END_DEV_POS_PAYMENT){
    ...
    if(txNew.vout.size() == 3){
        ...
        txNew.vout[2].scriptPubKey = GetScriptForDestination(DecodeDestination(FoundationPoSAddr));
        ...
        txNew.vout[3].scriptPubKey = GetScriptForDestination(DecodeDestination(FoundationPoSAddr));
...
int CMerkleTx::GetBlocksToMaturity() const
{
    if (!(IsCoinBase() || IsCoinStake()))
        return 0;
    return std::max(0, (Params().GetConsensus().nCoinbaseMaturity) - GetDepthInMainChain());
}
...
{
    return ((!IsLocked() && fWalletUnlockStakingOnly) || (!IsLocked() && !fWalletUnlockSperoSendOnly && !fWalletUnlockStakingOnly));
}

bool CWallet::IsUnLockedForSperosend()
{
    return ((!IsLocked() && fWalletUnlockSperoSendOnly) || (!IsLocked() && !fWalletUnlockSperoSendOnly && !fWalletUnlockStakingOnly));
}
