...
UniValue gettxout(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 3)
        throw std::runtime_error(
            ...
            "     \"addresses\" : [          (array of string) array of SperoCoin addresses\n"
...
// SperoCoin: Verify blockchain authenticity
UniValue verifyblockchain(const JSONRPCRequest& request)
{
    ...
    if(blockchainStatus == -2)
        result = "The authenticity of the SperoCoin blockchain has not yet been verified.";
    else if(blockchainStatus == -1)
        result = "The SperoCoin blockchain is not fully sychronized.";
    else if(blockchainStatus == 0)
        result = "The SperoCoin blockchain synchronized, but it does not match the latest checkpoint hash at Block " +
			std::to_string(LAST_REGISTERED_BLOCK_HEIGHT) +
			" (which is registered and guaranteed by the Bitcoin blockchain). So you are most likely on a forked chain, please resync with official peers at https://sperocoin.org.";
    else
        result = "The SperoCoin blockchain is fully synchronized. It is authentic! It is guaranteed by the Bitcoin blockchain (the most secure immutable database in the world) up to Block " +
            std::to_string(LAST_REGISTERED_BLOCK_HEIGHT) + ".";
