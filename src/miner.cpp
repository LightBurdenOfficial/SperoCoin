...
std::unique_ptr<CBlockTemplate> BlockAssembler::CreateNewBlock(const CScript& scriptPubKeyIn, bool fMineWitnessTx, bool fProofOfStake, CAmount* pFees)
{
    ...
    if((nHeight >= POS_POW_HYBRID && nHeight < HALVING_POW_03) || (nHeight > START_FOUNDATION_POW_BLOCKS && nHeight <= END_FOUNDATION_POW_BLOCKS)){
    ...
    if(!fProofOfStake) {
        ...
        std::string FoundationAddr = Params().GetFoundationAddr();
            if(nHeight >= POS_POW_HYBRID && nHeight < HALVING_POW_03){
                coinbaseTx.vout[0].scriptPubKey = scriptPubKeyIn;
                coinbaseTx.vout[1].scriptPubKey = GetScriptForDestination(DecodeDestination(FoundationAddr));
                coinbaseTx.vout[0].nValue = nFees + GetProofOfWorkReward(nHeight, pindexPrev) - devCoin;
                coinbaseTx.vout[1].nValue = devCoin;
            }

            if(nHeight >= START_FOUNDATION_POW_BLOCKS && nHeight <= END_FOUNDATION_POW_BLOCKS){
                coinbaseTx.vout[0].scriptPubKey = scriptPubKeyIn;
                coinbaseTx.vout[1].scriptPubKey = GetScriptForDestination(DecodeDestination(FoundationAddr));
                coinbaseTx.vout[0].nValue = devCoin - (GetProofOfWorkReward(nHeight, pindexPrev) + nFees);
                coinbaseTx.vout[1].nValue = devCoin;
            }
        /* Fim Adaptação para pagamentos Foundation */
    ...
    // SperoCoin: This is for SEGWIT and will activate a week before segwit - This requires all nodes to have been upgraded by then or they will fork.
...
bool SignBlock(CBlock& block, CWallet& wallet, CAmount nFees)
{
    ...
    // SperoCoin: In the old code, CTransaction sets nTime to GetAdjustedTime() when instantiated.
...
void Stake()
{
    ...
    while (true) {
        try {
            ...
            // SperoCoin: This appears to obtain a reserved script in
...
void StartThreadStake()
{
    ...
    stake_thread.reset(new boost::thread(boost::bind(&TraceThread<void (*)()>, "SperoCoin-Stake", &Stake)));
