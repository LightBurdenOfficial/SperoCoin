...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-checkblockchain", strprintf(_("Verifies the authenticity of the SperoCoin blockchain. (default: %u)"), DEFAULT_VERIFYBLOCKCHAINHASH));
#ifndef WIN32
    ...
    strUsage += HelpMessageOpt("-sperosendnode", strprintf(_("Participate in the SperoSend network and perform services for the network when possible. (default: %u)"), DEFAULT_SPEROSENDNODE));
    ...
    strUsage += HelpMessageOpt("-listenonion", strprintf(_("Automatically create Tor hidden service (default: %d)"), DEFAULT_LISTEN_SPERO));
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/DigitalCoin1/SperoCoin>";
    const std::string URL_WEBSITE = "<https://sperocoin.org>";
...
void ThreadImport(std::vector<fs::path> vImportFiles)
{
    ...
    RenameThread("SperoCoin-loadblk");
...
bool AppInitParameterInteraction()
{
    ...
    if (gArgs.GetBoolArg("-sperosendnode", DEFAULT_SPEROSENDNODE))
        nLocalServices = ServiceFlags(nLocalServices | NODE_SPERO_SEND);
...
bool AppInitMain()
{
    ...
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the "
                  "current working directory '%s'. This is fragile, because if SperoCoin is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if SperoCoin is started while in a temporary directory.\n",
    ...
    if (onionArg != "") {
        ...
        // SperoCoin always uses Tor, if not specified, connect to our default Tor node.
