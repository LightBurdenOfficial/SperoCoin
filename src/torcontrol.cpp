 ...
 * and ADD_SPERO (S3.27). See also sections 2.1 and 2.3.
 */
...
{
    if (reply.code == 250) {
        LogPrint(BCLog::TOR, "tor: ADD_SPERO successful\n");
        ...
        if (service_id.empty()) {
            LogPrintf("tor: Error parsing ADD_SPERO parameters:\n");
            for (const std::string &s : reply.lines) {
                LogPrintf("    %s\n", SanitizeString(s));
            }
            return;
...
void TorController::auth_cb(TorControlConnection& _conn, const TorControlReply& reply)
{
    if (reply.code == 250) {
        ...
        _conn.Command(strprintf("ADD_SPERO %s Port=%i,127.0.0.1:%i", private_key, GetListenPort(), GetListenPort()),
